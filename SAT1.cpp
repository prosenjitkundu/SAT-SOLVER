...
void codebreaker(int b, int w, int Choice[4])
{
    ...
    int i, j, k, l, n, m, p, q; 
    std::vector<int> clause;
    //The condition checking where if one color is present in
    //one position then the color must not present in  other position.
    for (int k = 0; k < 8; k++)
    {
        for (int j = 0; j < 4; j++)
        {
            for (int l = 0; l < 4; l++)
            {
                if (j == l)
                    break;
                clause.clear();
                clause.push_back(-(k * 4 + j));
                clause.push_back(-(k * 4 + l));
